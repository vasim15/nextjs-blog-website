    ...
    return (
        <div className='my-8 flex items-center justify-between border-b-2 border-gray-100'>
            <ul className='flex items-center'>
                ...
                {categories.map((category:ICategory) => (
                    <li key={category.id} className={`mr-6 pb-6 border-b-4 rounded-sm ${
                        isActiveLink(category) ? 'border-primary text-primary': 'border-white text-gray-400'
                    }`}>
            ...
            <svg
                ...
                <input type="text" placeholder='Search' className='outline-none px-2 py-1 ml-1' onChange={(e)=>handleOnSearch(e.target.value)} />
